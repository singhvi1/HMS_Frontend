...
import { setStudentVerificationSliceStatus } from "../utils/store/verificationRequestSlice";
...
const useVerificationStatus = () => {
    ...
    const verifyStudent = async (id, status) => {
        ...
        try {
            ...
            dispatch(setStudentVerificationStatus({ user_id: id, status }))
            dispatch(setStudentVerificationSliceStatus({ user_id: id, status }))
